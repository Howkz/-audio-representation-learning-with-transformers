...
\begin{itemize}
\item Screening sur LibriSpeech en sous-echantillons fixes pour controler le temps total.
\item Selection Top-5 executee en 2 seeds (42, 123) pour robustifier avant finale.
\item Final Top-3 execute en 5 seeds sur un sous-echantillon elargi (pas de full-data, budget 24h).
\item VoxPopuli est exclu du chemin principal pour rester compatible avec la contrainte temps.
...
\begin{itemize}
\item \textbf{Screening}: subsampled\_librispeech\_fixed\_budget.
\item \textbf{Consolidation finale}: expanded\_subsample\_librispeech\_fixed\_budget.
...
\paragraph{Justification du plan}
Etablir un point de reference frugal et stable compatible avec un budget <24h.
...
\paragraph{Variables manipulées}
\begin{itemize}
\item \texttt{training.pretrain.max\_steps=260}
\item \texttt{training.finetune.max\_steps=360}
\item \texttt{datasets.pretrain.max\_samples=2000}
\item \texttt{datasets.asr\_train.max\_samples=3000}
\item \texttt{datasets.asr\_valid.max\_samples=None}
\item \texttt{datasets.asr\_tests=[\{'name': 'openslr/librispeech\_asr', 'config': 'clean', 'split': 'test.clean', 'max\_samples': None, 'transcript\_key': 'text'\}]}
\end{itemize}
